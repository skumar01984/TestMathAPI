...
public class Assignment1_2_API_Validation extends Config {
	...
	public void postMathAPI() {
		System.out.println(postJsonExpressionData);
		response = given().auth().none().header("Content-Type", "application/json").contentType(ContentType.JSON).when()
				.body(postJsonExpressionData).post(postURL);
	...
	@Test
	public void validateExpressionData() {
